...
Replaces fixed percentage stops with time-aware, Greeks-aware exit logic
designed for the unique characteristics of same-day expiration options:
...
Exit hierarchy (checked in order):
    ...
    3. Catastrophic stop (large % loss, immediate exit)
    4. Profit target (time-scaled)
    5. Directional trailing stop (underlying-based)
    6. Greeks-aware stop loss
    7. Profitable hold timeout (take profits, don't let theta eat them)
"""
...
class ExitManager:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._max_hold_profitable = 20   # Exit winners before theta eats gains
        self._max_hold_absolute = 30     # Hard limit regardless of P&L

        # Catastrophic stop (always active, never adjusted)
        self._catastrophic_stop_pct = 0.50  # 50% premium loss = immediate exit
    ...
    def evaluate(self, ctx: ExitContext, now: Optional[datetime] = None) -> ExitDecision:
        ...
        # 6. Greeks-aware stop loss
        decision = self._check_greeks_stop(ctx, pnl_pct, hold_minutes)
        if decision.should_exit:
            return decision

        # 7. Profitable hold timeout
        if pnl_pct > 0.05 and hold_minutes >= self._max_hold_profitable:
            return ExitDecision(
                True,
                f"Profitable hold timeout ({hold_minutes:.0f}min, P&L {pnl_pct:.1%})",
                urgency="normal",
            )
    ...
    def _check_directional_trail(self, ctx: ExitContext, pnl_pct: float) -> ExitDecision:
        ...
        # Trail trigger: exit if >60% of favorable move retraced
        trail_threshold = 0.60
        if retracement >= trail_threshold and pnl_pct > -0.10:
            return ExitDecision(
                True,
    ...
    def _check_greeks_stop(
        self, ctx: ExitContext, pnl_pct: float, hold_minutes: float,
    ) -> ExitDecision:
        """Greeks-aware stop loss.

        High gamma (near ATM) = wider stop but tighter hold time.
        Low gamma (OTM) = tighter stop since delta exposure is fading.
        """
        gamma = ctx.contract.gamma if ctx.contract else 0.01
        base_stop = self._settings.sl_stop_loss_pct

        if gamma >= 0.03:
            # High gamma: near ATM, expect large swings
            # Give more room on % stop but enforce shorter hold
            stop_pct = base_stop * 1.5   # e.g., 30% → 45%
            max_hold = 15                # But only hold 15 min before cutting
        elif gamma >= 0.01:
            # Normal gamma: use base stop
            stop_pct = base_stop
            max_hold = self._max_hold_absolute
        else:
            # Low gamma: far OTM, delta is tiny — tighter stop
            stop_pct = base_stop * 0.67  # e.g., 30% → 20%
            max_hold = self._max_hold_absolute
        ...
        if pnl_pct <= -stop_pct:
            return ExitDecision(
                True,
                f"Greeks stop ({pnl_pct:.1%} <= -{stop_pct:.1%}, gamma={gamma:.3f})",
                urgency="urgent",
            )

        # High-gamma time limit on losers
        if gamma >= 0.03 and pnl_pct < 0 and hold_minutes >= max_hold:
            return ExitDecision(
                True,
                f"High-gamma hold limit ({hold_minutes:.0f}min, gamma={gamma:.3f}, P&L {pnl_pct:.1%})",
                urgency="urgent",
            )
