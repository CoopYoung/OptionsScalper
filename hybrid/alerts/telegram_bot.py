...
def _send_digest(digest: str) -> None:
    """Send a digest string to Telegram, handling HTML escaping and size limits."""
    escaped = html.escape(digest)
    text = escaped[:3900]
    if len(escaped) > 3900:
